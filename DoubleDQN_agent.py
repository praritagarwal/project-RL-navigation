...
class Agent():
    ...
    def learn(self, experiences, gamma):
        ...
        # note that for double DQN, we will choose the action using the local network and 
        # obtain its target q-value using the target network
        self.qnetwork_target.eval()
        self.qnetwork_local.eval()
        with torch.no_grad():
            next_actions = self.qnetwork_local(next_states).argmax(dim = 1).detach().view(-1,1)
            next_q_max = self.qnetwork_target(next_states).gather(1, next_actions).detach().view(-1,1)
